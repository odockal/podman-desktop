...
test('Verify main UI elements are present in Status Bar - Kind and Compose installation', async () => {
  if (existsSync(kindPath)) {
    ...
    await playExpect.soft(page.getByRole('listitem').filter({ hasText: 'Kind' })).toBeVisible();
  }
  await playExpect(page.getByRole('listitem').filter({ hasText: 'Compose' })).toBeVisible();
});
